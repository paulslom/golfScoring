...
{	 
	...
	private static void scan(DynamoDbTable<DynamoTeeTime> table) 
    {
		System.out.println("These are the contents of the teetimes table in dynamoDB");
		
        try 
        {
            Iterator<DynamoTeeTime> results = table.scan().items().iterator();
          	
            while (results.hasNext()) 
            {
                DynamoTeeTime rec = results.next();
                System.out.println("teeTimeID = " + rec.getTeeTimeID() + " .. gameID = " + rec.getGameID()
                		+ "  oldTeeTimeID = " + rec.getOldTeeTimeID() + " .. oldGameID = " + rec.getOldGameID()
                		+ "  playGroupNumber = " + rec.getPlayGroupNumber() + " .. teeTimeString = " + rec.getTeeTimeString());
            }
        } 
